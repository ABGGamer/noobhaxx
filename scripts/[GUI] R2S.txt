...
Decompile.FontSize = Enum.FontSize.Size10
Decompile.Text = "DECOMPILE (not supported)"
Decompile.TextColor3 = Color3.new(0.784314, 0.784314, 0.784314)
Decompile.TextSize = 10
...
local GetPath = function(Instance)
	local Obj = Instance
	if (Instance:GetFullName()) then
		return "game." .. Instance:GetFullName()
	else
		return ("nil -- Path contained an invalid instance")
	end
end
...
local on_namecall = function(object, ...)
	...
	local args = {...}
	args[#args] = nil
	if object.Name ~= "CharacterSoundEvent" and method:match("Server") and spy_enabled == true then get_namecall_dump(getfenv(0).script, object, unpack(args)) end
